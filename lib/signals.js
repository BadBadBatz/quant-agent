 ...
 * @param {object|number} rules - live thresholds, or legacy RSI upper bound
 */
export function evaluateCandidate(indicators, sentiment, currentPositions = [], rules = {}) {
  const { price, rsi, ma50, volume, avgVolume } = indicators;
  const sector = SECTOR_MAP[indicators.symbol] || 'unknown';
  const normalizedRules = typeof rules === 'number'
    ? { rsi_upper: rules }
    : rules || {};
  const rsiLower = Number(normalizedRules.rsi_lower ?? 25);
  const rsiUpper = Number(normalizedRules.rsi_upper ?? normalizedRules.rsiUpperBound ?? 40);
  const volumeMultiplier = Number(normalizedRules.volume_multiplier ?? 1.4);

  const results = {
    rsi_oversold: rsi > rsiLower && rsi < rsiUpper,
    above_ma50: SIGNALS.above_ma50(price, ma50),
    volume_spike: volume > avgVolume * volumeMultiplier,
  ...
  const signalDetails = [
    `Signal 1 (RSI oversold ${rsiLower}-${rsiUpper}): ${results.rsi_oversold ? '✓' : '✗'} RSI=${rsi}`,
    `Signal 2 (Price > MA50×0.97): ${results.above_ma50 ? '✓' : '✗'} Price=$${price}, MA50=$${ma50}`,
    `Signal 3 (Volume spike >${volumeMultiplier}×): ${results.volume_spike ? '✓' : '✗'} Ratio=${indicators.volumeRatio}x`,
